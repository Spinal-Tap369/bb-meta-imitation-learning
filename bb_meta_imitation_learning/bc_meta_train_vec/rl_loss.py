...
def reinforce_with_baseline(
    ...
    """
    REINFORCE + value baseline with optional off-policy correction.
    Returns (loss, mean_entropy, mean|adv|).
    """
    logp_cur = _log_probs_from_logits(cur_logits, actions)  # (T,)
    probs = F.softmax(cur_logits, dim=-1)
    logp_all = F.log_softmax(cur_logits, dim=-1)
    entropy = -(probs * logp_all).sum(dim=-1).mean()

    returns = discounted_returns(rewards, gamma)
    adv = gae(returns, values.detach(), gamma, lam) if use_gae else (returns - values.detach())
    ...
    if offpolicy != "none" and behavior_logits is not None:
        ...
        if offpolicy == "is":
            ...
            with torch.no_grad():
                v = values.detach()
                boot = torch.cat([v[1:], v[-1:]])  # shift with bootstrap on last
